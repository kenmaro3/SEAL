    ...
    */
    ...
    public:
        ...
        */
        template <
            typename T, typename = std::enable_if_t<
                            std::is_same<std::remove_cv_t<T>, double>::value ||
                            std::is_same<std::remove_cv_t<T>, std::complex<double>>::value>>
        inline void encode_as_coeff(
            const std::vector<T> &values, parms_id_type parms_id, double scale, Plaintext &destination,
            MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            encode_internal_as_coeff(values.data(), values.size(), parms_id, scale, destination, std::move(pool));
        }
        ...
        */
        ...
        inline void encode_as_coeff(
            ...
            MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            encode_as_coeff(values, context_.first_parms_id(), scale, destination, std::move(pool));
        }
#ifdef SEAL_USE_MSGSL
        /**
        Encodes a vector of double-precision floating-point real or complex numbers
        ...
        */
        template <
            typename T, typename = std::enable_if_t<
                            std::is_same<std::remove_cv_t<T>, double>::value ||
                            std::is_same<std::remove_cv_t<T>, std::complex<double>>::value>>
        inline void decode_as_coeff(
            const Plaintext &plain, std::vector<T> &destination, MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            destination.resize(2*slots_);
            decode_internal_as_coeff(plain, destination.data(), std::move(pool));
        }
#ifdef SEAL_USE_MSGSL
        /**
        Decodes a plaintext polynomial into double-precision floating-point
    ...
    private:
            ...
            MemoryPoolHandle pool)
        {
            ...
            {
                throw std::invalid_argument("values_size is too large from encode_internal");
            }
            if (!pool)
            {
            ...
            MemoryPoolHandle pool)
        {
            ...
            if (values_size > 2*slots_)
            {
                throw std::invalid_argument("values_size is too large from encode_internal_as_coeff");
            }
            if (!pool)
            {
                throw std::invalid_argument("pool is uninitialized");
        ...
        template <
            typename T, typename = std::enable_if_t<
                            std::is_same<std::remove_cv_t<T>, double>::value ||
                            std::is_same<std::remove_cv_t<T>, std::complex<double>>::value>>
        void decode_internal_as_coeff(const Plaintext &plain, T *destination, MemoryPoolHandle pool)
        {
            // Verify parameters.
            if (!is_valid_for(plain, context_))
            {
            ...
            for (std::size_t i = 0; i < 2*slots_; i++)
            {
                //double tmp = res[i].real();
                //if (tmp > 0.001 || tmp < -0.001){
                //   std::cout << res[i] << ", at " << i << std::endl;

                //}
                //destination[i] = from_complex<T>(res[static_cast<std::size_t>(matrix_reps_index_map_[i])]);
                destination[i] = from_complex<T>(res[static_cast<std::size_t>(i)]);
            }
        }
